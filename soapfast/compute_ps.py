...
if os.path.isfile(pname + "/utils/LODE/gvectors.so"):
    from .utils.LODE import PS_utils as psutil
    from .utils.LODE import parsing as parse
    has_gvector = True
else:
    from .utils import PS_utils as psutil
    from .utils import parsing as parse
# from .utils import PS_utils as psutil
# from .utils import parsing as parse

def get_power_spectrum(lam,frames,nmax=8,lmax=6,rc=4.0,sg=0.3,cw=1.0,periodic=False,sparse_options=[''],cen=[], spec=[],all_radial=[1.0,0.0,0.0],verbose=True,get_imag=False,norm=True,electro=False,sigewald=1.0,single_radial=False,radsize=50,lebsize=146,average=False,vol_frac=1.0):
    # set this to True because it's impossibly slow if set to False and give the
    # same output
    useall = True
    ...
    # Get list of species
    if (spec != []):
        all_species = list([atomic_numbers[sp] for sp in spec])
